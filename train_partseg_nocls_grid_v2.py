"""
Author: Benny (Modified with Scheduled Hard Masking & Loss Stabilization)
Date: 2025 Revised
"""
import argparse
import os
...
def plot_performance(exp_dir, history):
    ...
    plt.title('Performance Metrics (Stable Version)'); plt.ylim(-0.05, 1.05); plt.legend(loc='upper left', bbox_to_anchor=(1, 1)); plt.grid(True, alpha=0.3); plt.tight_layout()
...
def main(args):
    ...
    seg_weights = torch.Tensor([1.0, 40.0]).cuda() # 略微降低權重以穩定初期 Loss
    seg_criterion = torch.nn.NLLLoss(weight=seg_weights).cuda()
    pos_weight = torch.tensor([5.0]).cuda() # 顯著降低以提升 EmptyGrid IoU
    ...
    for epoch in range(args.epoch):
        ...
        for i, (points, label, target, grid_gt) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            total_loss = loss_seg + 1.0 * loss_grid # 降低輔助任務權重
            total_loss.backward(); optimizer.step()
            train_loss_epoch.append(total_loss.item()); grid_loss_epoch.append(loss_grid.item())
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target, grid_gt) in enumerate(testDataLoader):
                ...
                # --- Scheduled 硬遮罩過濾 ---
                if epoch > 100:
                    grid_mask = (torch.sigmoid(grid_pred) > 0.1).float()
                    points_np, grid_mask_np = points.numpy(), grid_mask.cpu().numpy()
                    for b in range(cur_batch_size):
                        for n in range(NUM_POINT):
                            if cur_pred_val[b, n] == 1:
                                xyz = points_np[b, n]
                                grid_size = 2.0 / args.grid_num
                                gi = int((xyz[0] + 1.0) / grid_size)
                                gj = int((xyz[1] + 1.0) / grid_size)
                                gk = int((xyz[2] + 1.0) / grid_size)
            ...
            log_string(f'Grid Branch -> Acc: {history["test_grid_acc"][-1]:.4f}, GateGrid IoU: {history["test_grid_iou"][-1]:.4f}, EmptyGrid IoU: {history["test_empty_grid_iou"][-1]:.4f}')
