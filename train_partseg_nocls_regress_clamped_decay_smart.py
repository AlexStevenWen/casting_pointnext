...
def get_spatial_metrics(points, preds, targets, threshold=0.5):
    ...
    gt_max_idx = torch.argmax(targets, dim=1)
    
    pred_xyz = points[torch.arange(batch_size), pred_max_idx, :]
    gt_xyz = points[torch.arange(batch_size), gt_max_idx, :]
    
    argmax_dists = torch.norm(pred_xyz - gt_xyz, dim=1)
    
    # 2. Weighted Center Shift
    weighted_dists = []
...
def main(args):
    ...
    # ===================================================
    # [新增] Smart Rewind & Punish 機制變數初始化
    # ===================================================
    patience_counter = 0
    patience_limit = 15          # 容忍次數：連續 15 個 Epoch 沒進步就觸發
    overfit_protection_count = 0 
    max_protections = 30          # 最多救 3 次
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            # ===========================================================
            # [新增] Smart Rewind & Punish 邏輯
            # ===========================================================
            if epoch_mae <= best_mae:
                best_mae = epoch_mae
                patience_counter = 0 # 表現好，計數器歸零
                ...
                if patience_counter >= patience_limit:
                    if overfit_protection_count < max_protections:
                        ...
                        # 1. Rewind: 讀取上次最好的模型
                        # 加入 weights_only=False 以允許讀取包含 Numpy 數值的 Checkpoint
                        ...
                        # 2. Punish: 加重 Weight Decay (乘2)
                        current_weight_decay = current_weight_decay * 2.0 
                        current_weight_decay = min(current_weight_decay, 0.1) # 上限保護
                        ...
                        # 3. Warm Restart: 重啟學習率 (設為初始值的 50%)
