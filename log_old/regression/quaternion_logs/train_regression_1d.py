...
def test(model, loader, criterion, args):
    ...
    with torch.no_grad(): 
        for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
            ...
            if args.output_dim == 4:
                ...
                abs_dot = torch.clamp(torch.abs(dot_product), -1.0, 1.0)
                angle_rad = 2.0 * torch.acos(abs_dot)
                angle_deg = torch.rad2deg(angle_rad)
                all_errors_deg_list.append(angle_deg.cpu().numpy())

            else:
                # (模式 2: 歐拉角) 計算 MAE (度)
                # (假設 pred 和 target 都在 [-1, 1] 範圍)
                abs_error_normalized = torch.abs(pred - target)
                abs_error_deg = abs_error_normalized * 180.0
                all_errors_deg_list.append(abs_error_deg.cpu().numpy())
    # --- (*** 新增：計算最終指標 ***) ---
    ...
    # --- (*** 關鍵修改：計算 MAE ***) ---
    all_errors_deg = np.concatenate(all_errors_deg_list, axis=0)

    # 2. 計算 MAE (in Degrees)
    mae_deg = np.mean(all_errors_deg)

    # 3. 計算 準確率@5度
    threshold_deg = 5.0
    acc_5_deg = np.mean(all_errors_deg <= threshold_deg)

    # 4. (可選) 計算 準確率@5度 (Per Object)
    if args.output_dim == 4:
        acc_obj_5_deg = acc_5_deg # (四元數只有 1 個總角度)
    else:
        # (歐拉角有 N 個軸)
        acc_obj_5_deg = np.mean(np.all(all_errors_deg <= threshold_deg, axis=-1))
    # --- (*** 修改結束 ***) ---

    return mean_loss, mae_deg, acc_5_deg, acc_obj_5_deg
...
### 6. 修改驗證迴圈 (Validation Loop Change) ###
        with torch.no_grad():
            # 1. 確保您正確解開了 4 個回傳值
            val_loss, val_mae_deg, val_acc_axis, val_acc_obj = test(classifier.eval(), testDataLoader, criterion, args) 

            # 2. (*** 關鍵修正 ***) 
            # 判斷標準必須是 val_mae_deg (角度誤差)，而不是 val_loss
            is_best_model = val_mae_deg < best_loss 
            if is_best_model:
                best_loss = val_mae_deg # <-- 儲存 50.76，而不是 0.20
                best_epoch = epoch + 1
            
            # 3. (*** 關鍵修正 ***)
            # 印出 best_loss (它現在存的是 50.76)
            log_string('Validation Loss: %f' % val_loss)
            if args.output_dim == 4:
                log_string('Validation MAE (Angular Deg): %.2f' % val_mae_deg)
                log_string('Accuracy (Angle < 5°): %.4f' % val_acc_axis)
                log_string('Best MAE (Angular Deg): %.2f (at epoch %d)' % (best_loss, best_epoch))
            else:
            ...
            if (is_best_model): 
                ...
                state = {
                    'epoch': best_epoch,
                    'val_loss': val_loss, 
                    'val_mae_deg': val_mae_deg, # (新) 儲存 MAE
