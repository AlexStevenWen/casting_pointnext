...
def test(model, loader, criterion, args): # <--- 修正：傳入 args
    model.eval()
    total_loss = 0.0
    num_batches = 0
    ...
    mean_loss = total_loss / float(num_batches) if num_batches > 0 else 0.0 # <--- 修正：避免除以零
    return mean_loss
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            val_loss = test(classifier.eval(), testDataLoader, criterion, args) # <--- 修正：傳入 args
            ...
            log_string('Best Validation Loss: %f (at epoch %d)' % (best_loss, best_epoch)) 
