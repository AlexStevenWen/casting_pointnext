...
class get_loss(nn.Module):
    def __init__(self, smoothing=0.2): # 論文建議值 0.2
        super(get_loss, self).__init__()
        self.smoothing = smoothing

    def forward(self, x, xyz):
        identity = x
        # 注意：這裡中心點和搜尋點都是 xyz [B, 3, N]
        n_idx = ball_op(self.radius, self.k, xyz.transpose(1, 2), xyz.transpose(1, 2))
        
        # 確保 gather 後的數據正確
        neighbor_xyz = gather(xyz, n_idx) # [B, 3, N, K]
        # (絕對座標 - 中心點座標) / 半徑
        neighbor_xyz = (neighbor_xyz - xyz.unsqueeze(-1)) / self.radius
        
        neighbor_feat = torch.cat([gather(x, n_idx), neighbor_xyz], dim=1) # [B, D+3, N, K]
