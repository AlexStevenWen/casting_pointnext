...
class get_model(nn.Module):
    ...
    def forward(self, xyz):
        ...
        grid_logits = self.grid_head(torch.cat([feat_mid, feat_deep], dim=1)) # [B, 64]
        grid_probs = torch.sigmoid(grid_logits) # [B, 64]

        # 4. 核心：根據點的位置查詢對應格子的機率 (Spatial Gate Attention)
        # 將 XYZ 從 [-1, 1] 映射到 [0, grid_num-1]
        gn = self.grid_num
        norm_xyz = (l0_xyz.transpose(1, 2) + 1.0) / 2.0 # [B, N, 3]
        grid_coords = (norm_xyz * gn).long().clamp(0, gn - 1)
        
        # 轉為 1D 索引: idx = x*n^2 + y*n + z
        point_in_grid_idx = grid_coords[:, :, 0] * (gn**2) + \
                            grid_coords[:, :, 1] * gn + \
                            grid_coords[:, :, 2] # [B, N]
        ...
        # 6. 注入注意力：局部特徵加強
        # f = f * (1 + Gate)
        l0_points = l0_points * (1.0 + point_weights)
...
class get_loss(nn.Module):
    def __init__(self, weight=None):
        ...
        self.weight = weight
        # 針對 136 件小樣本，降低 pos_weight 至 5.0 以提升 EmptyGrid IoU (泛化關鍵)
        self.grid_criterion = FocalLossBCE(pos_weight=torch.tensor([5.0]).cuda())

    def forward(self, pred, target, grid_pred, grid_target):
        # 點分割損失 (Label Smoothing 0.2 強制平滑預測信心)
        seg_loss = F.nll_loss(pred, target, weight=self.weight, label_smoothing=0.2)
        
        # 格子損失 (權重比例維持 1.0，因為現在有 Attention 連動，梯度會自動校準)
