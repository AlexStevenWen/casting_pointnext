...
class RegressionDataLoader(Dataset):
    def __init__(self, root, args, split='train', process_data=False):
        ...
        # 3. (新) 獲取 CSV 中的 *所有* 標籤欄位
        all_label_columns = [col for col in self.data_frame.columns if col != 'filename']
        
        # 4. (新) 根據 args.output_dim 決定 *真正要載入* 的標籤欄位
        if hasattr(args, 'output_dim'):
            # (*** 新增 Quaternion 選項 ***)
            if args.output_dim == 4 and 'qw' in all_label_columns:
                print(f"Info: output_dim={args.output_dim}。將只載入 ['qw', 'qx', 'qy', 'qz'] 欄位。")
            self.label_columns = ['qw', 'qx', 'qy', 'qz']
            if args.output_dim == 3 and 'rx' in all_label_columns:
                print(f"Info: output_dim={args.output_dim}。將只載入 ['rx', 'ry', 'rz'] 欄位。")
                self.label_columns = ['rx', 'ry', 'rz']
            elif args.output_dim == 1 and 'ry' in all_label_columns:
                print(f"Info: output_dim={args.output_dim}。將只載入 ['ry'] 欄位。")
                self.label_columns = ['ry']
            else:
                # 保持原始行為 (例如 output_dim=6)
                print(f"Info: output_dim={args.output_dim}。將載入全部 {len(all_label_columns)} 個欄位。")
                self.label_columns = all_label_columns
        else:
            # 如果 args 中沒有 output_dim，載入全部
            self.label_columns = all_label_columns
            
        # 5. (新) 檢查最終選擇的欄位數是否匹配
        if hasattr(args, 'output_dim') and args.output_dim != len(self.label_columns):
            print(f"警告: 模型的 output_dim ({args.output_dim}) 與實際載入的標籤數 ({len(self.label_columns)}) 嚴重不符。")
            print(f"    CSV 欄位: {all_label_columns}")
            print(f"    選擇的欄位: {self.label_columns}")

        # --- (*** 修改結束 ***) ---

        print(f'The size of {split} data is {len(self.data_frame)}. Labels: {self.label_columns}')

        # 6. (新) 適應 process_data (預處理緩存)
        if self.process_data:
            # ( ... 這一整段 process_data 的程式碼保持不變 ... )
    ...
    def _get_item_from_disk(self, index):
        ...
        if 'qw' in self.label_columns:
                # (模式 1: 四元數) L2 歸一化
                norm = np.linalg.norm(label)
                if norm > 1e-6:
                    label = label / norm
                else:
                    label = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)

        elif 'rx' in self.label_columns and len(self.label_columns) == 3:
            # (模式 2: 歐拉角 3D) 除以 pi
            label = label / np.pi

        elif 'rx' in self.label_columns and len(self.label_columns) == 6:
            # (模式 3: 歐拉角 6D) 只歸一化後 3 個
            label[3:] = label[3:] / np.pi
        # 2. 讀取 H5 檔案
